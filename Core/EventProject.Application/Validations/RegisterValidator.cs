...
namespace EventProject.Application.Validations
{
    public class RegisterValidator : AbstractValidator<RegisterRequest>
    {
        public RegisterValidator()
        {
            ...
            RuleFor(x => x.Fistname).NotEmpty().WithMessage("Ad boş olmamalidir").MinimumLength(3).WithMessage("Ad minimum 3 simvol olmalidir").Must(name => char.IsUpper(name[0])).WithMessage("Ad böyük hərflə başlamalıdır!"); 
