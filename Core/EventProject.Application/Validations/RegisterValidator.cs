...
namespace EventProject.Application.Validations
{
    public class RegisterValidator : AbstractValidator<RegisterRequest>
    {
        public RegisterValidator()
        {
            ...
            RuleFor(x => x.Fistname).NotEmpty().WithMessage("Ad boş olmamalidir").MinimumLength(3).WithMessage("Ad minimum 3 simvol olmalidir").Must(name => name.Length > 0 && char.IsUpper(name[0])).WithMessage("Ad böyük hərflə başlamalıdır!"); 

            RuleFor(x => x.Lastname).NotEmpty().WithMessage("Soyad   boş olmamalidir").MinimumLength(3).WithMessage("Soyad minimum 4 simvol olmalidir").Must(x =>x.Length>0&& char.IsUpper(x[0])).WithMessage("Soyad böyük hərflə başlamalıdır!");
